...
function Login() {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    if(formData.email !== "" && formData.password !=="" && formData.email.includes('@') && formData.password.length >= 8){
  ...
  const loginWithTestHandler = () =>{
    setFormData(formData => ({...formData , email : "test@gmail.com" ,password : "Test@123"}));
    ...
    <div className='container flex mx-auto items-center h-screen max-w-2xl 3xl:max-w-3xl lg:justify-center'>
      ...
      <div className='flex flex-col w-2/5 lg:w-3/5 '>
         ...
         <img src={logoLg} alt="logo image"/>
          ...
          <div className='text-center mx-3  w-full my-2 text-red-500'>{validateCred}</div>
          ...
          <input
                  ...
                  />
                   <button className="bg-purple-500 text-white w-full rounded h-8 font-bold mx-3 py-1 px-3 my-3 hover:bg-purple-600">Login</button>
          ...
          <div className='text-center mx-3 py-2 px-3 w-full my-2'>Dont have account?<span className='font-semibold hover:underline cursor-pointer'> <Link to="/signup">Sign Up</Link></span></div>
        </div>
